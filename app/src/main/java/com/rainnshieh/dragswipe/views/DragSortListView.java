...
import com.rainnshieh.dragswipe.R;
...
public class DragSortListView extends ListView {
     ...
     */
    public boolean mLastCallWasIntercept = false;

    /**
     * A touch event is in progress.
     */
    public boolean mInTouchEvent = false;
    ...
    public static final int ON_INTERCEPT_TOUCH_EVENT = 2;

    /**
     * Where to cancel the ListView action when a
     * drag-sort begins
     */ 
    public int mCancelMethod = NO_CANCEL;
     ...
     * playing.
     */
    public boolean mIgnoreTouchEvent = false;
    ...
    public void doActionUpOrCancel() {
    ...
    public void saveTouchCoords(MotionEvent ev) {
        int action = ev.getAction() & MotionEvent.ACTION_MASK;
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        ...
        switch (ev.getAction()){
            ...
            case  MotionEvent.ACTION_MOVE:
                float moveY = Math.abs(ev.getRawY() - mDownY);
                float moveX = Math.abs(ev.getRawX() - mDownX);
                if(moveY - moveX > 5){//上下拖动
                    intercept = true;
                }
                mDownX = ev.getRawX();
                mDownY = ev.getRawY();
